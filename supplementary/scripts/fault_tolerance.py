"""
Fault-tolerance experiment: EigenTree-UCB with hub failures injected at fixed
rounds, recovered by warm-started re-election. See the paper's Fault Tolerance
section for the procedure; this module runs it and plots group regret through
the failures.

Usage:
    python fault_tolerance.py --graph er --N 20 --K 10 --T 2000 --fail-at 500 1000 1500 --mode all
"""
...
# ============================================================
#
# Both decentralized budgets are ceil(2 ln N), the protocol's O(log N) spec.
# They are fixed in advance from the known N, not derived at runtime, so every
# node uses the same value and stays synchronized. The constant 2 is empirical:
# past it, more rounds buy little extra agreement with a fully-converged hub.
...
def gossip_power_iteration(G, w0, tau, tol=None, gossip_rounds=None):
    """
    Decentralized power iteration with gossip-based normalization, used for both
    the initial centrality and fault-tolerant re-election.

    Two modes, set by `tol`:
      tol is None (the protocol): run exactly `tau` rounds with no convergence
        test, so all nodes finish on the same round and stay synchronized.
      tol is a float (measurement only): stop early when the per-node relative
        change falls below tol, and report the round. Not used by the protocol.

    For re-election on a surviving subgraph, pass the induced subgraph and a w0
    equal to the pre-failure state on the live nodes (warm start). Returns
    (w, rounds); `gossip_rounds` defaults to ceil(ln N).
    """
    N = G.number_of_nodes()
...
def run_eigentree_ft(env, T, G, N, t_fail, c=2.0, sigma=1.0, tau_init=None,
    ...
    """
    Runs the EigenTree-UCB relay with permanent hub failures injected at the
    rounds in t_fail (an int or a list). Recovery is the re-election procedure
    of the paper's Fault Tolerance section.

    enable_failover=False keeps the hub dead forever after the first failure,
    isolating what re-election buys.

    Returns:
        cum_regret: (N, T) array of per-agent cumulative regret.
        events: dict with 'hub_history', 'recovery_rounds' (round each new hub
                resumed cycling), and 'tree_valid_checks' (validity dict per
                re-election).
    """
    K = env.K
    if tau_init is None:
    ...
    # Each node's counters as of its last uplink fold. Folding only the
    # increment since this snapshot keeps the aggregate a true running total
    # that survives a change of hub, unlike re-deriving it from n_own.
    ...
    while t < T:
        ...
        # Detection: the uniform timer tau = 2D+1 (one cycle) fires when an
        # expected message is missed, so the hub's children catch its death
        # within a cycle. Same timer every node runs against every neighbor.
        ...
        if detected and enable_failover and not reelection_in_progress:
            ...
            # No backup is mirrored: re-sum the survivors' own counters into the
            # new hub immediately, so the transient below is pure recovery delay
            # (detection + re-election + rebuild), not bandit-state staleness.
            ...
            # Transient = failure-notice horizon (2*D before the failure)
            # + warm-started re-election + max-flood rebuild (<= 2*new_D).
            horizon = 2 * D_before
            rebuild = 2 * new_D
