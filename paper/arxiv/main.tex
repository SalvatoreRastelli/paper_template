...
\Repeat
    ...
    \State \textcolor{gray}{$\triangleright$ \textbf{Downlink} ($D$ rounds)}
    \State send $(m,\, D,\, \mathrm{done},\, \mathrm{hops}{=}0)$ to each child in $\mathrm{ch}(h^\star)$
    \For{$d = 1$ \textbf{to} $D$}
        \ForAll{$i \in V$ with $d_i = d$}
            \State on receipt of $(m, D, \mathrm{done}, \mathrm{hops})$: update $\sigma_i$ from $m$; forward $(m, D, \mathrm{done}, \mathrm{hops}{+}1)$ to each child in $\mathrm{ch}(i)$ \Comment{$i$ never independently evaluates the stopping condition; it only learns $\mathrm{done}$ from the packet}
        \EndFor
        \State $t \mathrel{+}= 1$
    \EndFor
