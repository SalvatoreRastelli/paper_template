...
\label{cor:bai}
The total number of arm pulls required by EigenTree-BAI to identify $a^\star$ with probability $\geq 1-\delta$ is identical to the centralized algorithm of \citet{hillel2013distributed}; the only overhead is $2D+1$ extra time steps per elimination round.
\end{corollary}
...
\texttt{fault\_tolerance.py}) are self-contained and share no imports.
...
\paragraph{Parameters.} All experiments use the settings stated in the main
text: for regret minimization, $N=100$, $K=5$, $T=5000$, $p=0.5$, 50 runs; for
