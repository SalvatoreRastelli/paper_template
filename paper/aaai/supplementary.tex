...
After any single node failure, the warm-started power iteration on $\tilde A$ converges to the leading eigenvector of $\tilde A$ from the pre-failure $\tilde\psi$, and reaching tolerance $\epsilon$ takes $O\bigl(\log(\sqrt{\deg(v)}/(\gamma\epsilon))/\log(\lambda_1/\lambda_2)\bigr)$ rounds, where $\gamma$ is the eigenvalue gap of $\tilde A$. On a graph with $\lambda_2$ bounded away from $0$, this is $O(\log(N/\epsilon))$.
\end{proposition}
...
For the rate, the initial angle to the target is bounded by Davis--Kahan
\citep{daviskahan1970},
$\sin\Theta(\tilde\psi,\tilde\psi_{\tilde A}) \leq \|E\|_2/\gamma
= \sqrt{\deg(v)}/\gamma$. Each power-iteration round multiplies the
component orthogonal to $\tilde\psi_{\tilde A}$ by at most $\lambda_2/\lambda_1$
relative to the leading one, so the residual angle after $t$ rounds is at most
$(\lambda_2/\lambda_1)^t\tan\Theta$. Setting this below $\epsilon$ gives
$t = O(\log(\tan\Theta/\epsilon)/\log(\lambda_1/\lambda_2))
   = O(\log(\sqrt{\deg(v)}/(\gamma\epsilon))/\log(\lambda_1/\lambda_2))$.
When $\lambda_2$ is bounded away from $0$, both $\gamma$ and
$\log(\lambda_1/\lambda_2)$ are $\Omega(1)$ while $\deg(v)\leq N$, so this
is $O(\log(N/\epsilon))$.
\end{proof}
