...
Formally, let $A \in \{0,1\}^{N \times N}$ be the adjacency matrix of $G$. By the Perron--Frobenius theorem \citep{hornjohnson2013}, $A$ has a unique positive leading eigenvalue $\lambda_1$ with strictly positive eigenvector $\psi$, $\|\psi\|_2 = 1$; MERW's stationary distribution is $\pi_i = \psi_i^2$. We use $\psi_i$ as node $i$'s centrality score.

No single agent has access to $A$, so $\psi$ is computed by the distributed power iteration of \citep{jelasity2007asynchronous}. Each agent $i$ keeps a local scalar $w_i$, initialized to 1, and repeats: (1) collect $w_j$ from every neighbor and form the unnormalized update $\tilde w_i = \sum_{j\sim i} w_j$; (2) compute the local log-growth rate $r_i = \log(\tilde w_i/w_i)$; (3) average $r_i$ with random neighbors for $O(\log N)$ gossip rounds, so that every $r_i$ converges to the same network-wide estimate of $\log\lambda_1$ without any node ever summing over all of $N$; (4) normalize locally, $w_i \leftarrow \tilde w_i/\exp(r_i)$. This gossip step replaces the single global $\ell_2$-norm renormalization an ordinary (non-distributed) power iteration would need, at the cost of the extra gossip rounds. Since only the \emph{ratio} $\psi_j/\psi_i$ is needed for routing (agent $i$ simply checks which neighbor's value is larger), the common unknown scale $\tilde\psi_i \approx c\cdot\psi_i$ left behind by this normalization is never resolved and never needs to be: no agent ever learns $N$ or $c$.

After power iteration, a short \emph{max-flooding} phase merges any local maxima of $\tilde\psi$ into a single spanning tree. Each agent $i$ keeps a flooded value $m_i$, initialized to $\tilde\psi_i$, and a pointer $\mathrm{via}_i$ initialized to itself. Every round, $i$ broadcasts $m_i$ to its neighbors; if some neighbor $j$ reports $m_j > m_i$, $i$ sets $m_i \leftarrow m_j$ and $\mathrm{via}_i \leftarrow j$. Once a round passes in which no agent updates its value, the flood has converged, purely a local observation, with no agent needing to know $\mathrm{diam}(G)$ in advance. At convergence, $m_i$ equals the global maximum $\tilde\psi_{h^\star}$ everywhere, and $\mathrm{via}_i$ points one step closer to $h^\star$; any node whose own $\tilde\psi_i$ was a local but not the global maximum re-routes toward $\mathrm{via}_i$, while $h^\star$ itself, whose value never has a better neighbor, keeps no parent and becomes the root. The result is a single directed spanning tree $\mathcal{T}$ rooted at $h^\star = \arg\max_i \tilde\psi_i$, converging in at most $\mathrm{diam}(G)$ rounds. Immediately afterward, each node $i$ knows only its own parent $\mathrm{par}(i)$; its children, its depth $d_i$, and the tree height $D$ are all discovered incrementally during the protocol's first cycle below, not known in advance.

Complete proofs of every theorem, proposition, and corollary in this paper are given in the supplementary material, using matching notation and statement numbering; we give the proof of Theorem~\ref{thm:coordinator} here as well, since it is the paper's central claim.
