...
Theorem~\ref{thm:mdst-gap} already gives a route to a high-probability guarantee on random graphs, without needing a new proof: on an Erd\H{o}s--R\'{e}nyi graph $G(n,p)$ with $p$ bounded above $1/2$ by a constant margin, every node's degree concentrates tightly around its mean $(n-1)p$, so $\deg(h^\star) + \delta_{\bar N}(h^\star)$ exceeds $n$ with probability approaching 1 as $n$ grows, placing the graph inside Theorem~\ref{thm:mdst-gap}'s hypothesis and forcing $D \leq 2$. So on dense enough Erd\H{o}s--R\'{e}nyi graphs, \merw{}'s tree is, with high probability, no more than one hop deeper than a true minimum diameter spanning tree. A complete, general relation between $D$ and $D'$ that covers sparser graphs and other random graph models without this density margin is left for future work.
