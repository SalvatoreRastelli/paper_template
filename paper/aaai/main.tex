...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.95\columnwidth]{results/MERW_visualization/merw_tree_er_N20.pdf}
    \caption{MERW centrality and routing tree on an Erd\H{o}s--R\'{e}nyi graph ($N=20$). \emph{Left}: undirected graph, nodes colored by $\psi_i/\psi_{\max}$. \emph{Center}: routing subtree overlaid on the graph, hub ringed in gold. \emph{Right}: induced directed tree, hierarchical layout, arrows child $\to$ parent.}
...
\paragraph{2-connected graphs.} Theorem~\ref{thm:mdst-gap}'s hypothesis is a strong density requirement. A weaker, purely topological condition -- $G$ remaining connected after removing any single node -- is also the exact requirement Section~\ref{sec:ft} places on $G$ for hub failover to work at all, so it is already assumed elsewhere in this paper.

\begin{conjecture}[Depth gap under 2-connectivity]
\label{conj:mdst-2conn}
If $G$ is 2-connected, then $D \leq D' + 1$.
\end{conjecture}

We tested Conjecture~\ref{conj:mdst-2conn} against counterexamples on over 8000 random 2-connected Erd\H{o}s--R\'{e}nyi graphs, $n \in [5,50]$ and $p \in [0.05, 0.6]$, including hand-constructed theta graphs (a cycle plus one extra path joining two of its nodes) and dense-cluster-with-two-disjoint-paths graphs designed to stress the bound: no violation was found. We were not able to prove it, and are not aware of an existing theorem of this form; the closest related result bounds any vertex's eccentricity by twice the radius with no connectivity assumption \citep{buckleyharary1990}, a substantially weaker guarantee. We leave the proof as an open problem.
...
This is a strong requirement -- every node needs a neighborhood covering roughly half the network -- but it holds without exception once satisfied, unlike the empirical regularity of Conjecture~\ref{conj:mdst-2conn} below.
